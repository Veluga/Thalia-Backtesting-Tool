...
\label{Harvester}
All financial backtesting application rely on having accurate data to work with. We required a system that can centralize the data from multiple APIs and to keep our database updated with live data.
The requirements for such a system were:
\begin{itemize}
\item \textbf{Standard data format:} In order to be able to store the data into a database with Finda and have the financial analysis done by Anda the Harvester needs to put all the data received trough the APIs in a standard format without losing numerical accuracy or granularity.
\item \textbf{Update the database:} The database has to be updated regularly in order to contain live data.
\item \textbf{Data Interpolation:} Some financial assets require interpolation in order to account for the days without any transactions.
\item \textbf{Redundancy:} We have no power over the APIs we are using. The required system needs to have redundancy measures so that we can still retrieve live data even if a API fails.
\item \textbf{Cross Checking capabilities:} Comparing data from multiple source brings us a advantage over all other competitors.
\item \textbf{Maintenance:} The financial world is in a continuous change. Because of this it is important to make sure that we are able to add and delete assets as the time goes on.
\end{itemize}

We started our quest for data by trying to find APIs that could call for the main financial assets used when making a investment portfolio. We observed that for cryptographic and normal currencies we can use \textbf{nomics}. We chose nomics because compared to other APIs it was the only one to give free live data for a unlimited amount of time. For the rest of the assets and more we found that \textbf{pandas\_datareader} provides access to a large number of APIs that also happen to be the biggest APIs for financial data available on the internet. Pandas Datareader gives us the possibility of implementing the redundancy and cross checking capabilities that we desire. In addition to this pandas\_datareader is the only option on the market that manages to standardise the calling procedure for multiple financial APIs.

For the rest of the requirements of the data collection system we have seen that there is already existing systems that can do what we want. As a result we had to write our own system.
In the building of the system we have used python as a programming language. This decision has been made because python is the language with most APIs for financial data. In order to manipulate the data we have used the pandas library. The persistent data required by the update mechanism is stored in CSV format because of the readability and the ease of use when using pandas.
