...
\item \textbf{Data Interpolation:} Some financial assets require interpolation to account for the days without any transactions.
...
We started our quest for data by trying to find APIs that could call for the main financial assets used when making an investment portfolio.
...
\textbf{FOREX API\newline}
Between nomics and fixerio we have chosen nomics for the simple fact that nomics had unlimited calls for free, besides, nomics also has cryptocurrencies data. However, Fixerio is a more robust API with a larger user base and more data, that we would have used if could have afforded the investment. In the scenario where we would have had money to spend we would have used fixerio for currencies and nomics for cryptocurrencies. However, in our case we have no money so we used nomics for both currencies and cryptocurrencies.\newline

\textbf{Stock Market Data\newline}
Out of yfinance, Alpha Vantage and Quandl we have chosen yfinance because it offered the most data and API calls for free, compared to the other two. If we had money to spend we would have used Alpha Vantage because of the larger number of available assets and the high number of requests per minute. As for Quandl we found it to be expensive and not centered on the type of data we need.\newline

\textbf{Standard Data Format\newline}
While looking for solutions that would help with solving the data format and the maintenance problem we have found pandas\_datareader. It is a wrapper for the biggest financial APIs. We saw that it included all of the Stock Market Data APIs named above and more. 
In addition, it standardizes the calling procedure for all APIs it includes and it returns the same format no matter the API you are calling. This is the only product that does this on the market so no compassion with other product can be done. In the case where we would have not used it we would have produced code that would have achievedd the same result. In conclusion, using pandas\_datareader has saved us some valuable man-hours.\newline
