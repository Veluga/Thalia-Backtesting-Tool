...
For the rest of this section we shall assume that the application was launched on a computer, although the layout is identical and intuitive in both cases. 
...
As we can see we managed to encapture the overall look as we envisioned with minor changes only. The data is significantly better distributed by the use of tabs, giving a less cluttered UI. In addition, we have decided not to implement the aside menu, partly because it was unnecessary, but partly because it suggests that the backtest results can be changed on the go. This, as we have realised was not so feasible, due to excessive calculations in the Business Logic Layer.

One change we had to make was the use of the Dash table instead of the approach visible on \figurename{\ref{wireframe}}. The benefits of this change as previously discussed. In addition, we have also decided to discard the sliders as a way of changing asset allocations.
