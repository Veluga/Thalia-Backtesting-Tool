...
|write_asset_values(values)| Write asset values specified in pandas.DataFrame values to database. If any of the records already exist, quietly overwrite them. <br> - If added data would generate 'holes in the data' (the series of dates an assetTicker has data stored for would no longer be continuous), the write will fail. <br> - If any of the records reference an AssetTicker not in any record in Asset table, module will complain. <br> Input pandas.Dataframe of format: <br> `{Columns: [AOpen<Decimal.decimal>, AClose<Decimal.decimal>, AHigh<Decimal.decimal>, ALow<Decimal.decimal>] Index: [AssetTicker<String>, ADate <datetime.date>]}`|
|write_dividend_payouts(payouts)|takes as parameter pandas.DataFrame of format: <br> ` {Columns: [Payout<decima.Decimal> Index: [AssetTicker<String>, PDate<datetime.date>]}` <br> Records are written to the database, checking that foreign key constraint on Asset.AssetTicker is not violated|
